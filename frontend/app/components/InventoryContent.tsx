...
// Mock warehouse locations for demo
const WAREHOUSE_OPTIONS = ["Kho A", "Kho B", "Kho C"];
...
export default function InventoryContent() {
  ...
  const mapVehiclesToRows = useCallback((vehicles: VehicleListResponse[]) => {
    return vehicles.map<InventoryTableRowData>((vehicle, index) => ({
      id: vehicle.id,
      ...
      vin: vehicle.vin ?? "-",
      color: vehicle.color ?? "-",
      arrivalDate: vehicle.manufactureDate
        ? new Date(vehicle.manufactureDate).toLocaleDateString("vi-VN")
        : "-",
      condition: "Tốt",
      warehouse: WAREHOUSE_OPTIONS[index % WAREHOUSE_OPTIONS.length],
      status: vehicle.status ?? "AVAILABLE",
      statusLabel: vehicle.status === "AVAILABLE" ? "Đang tồn" : "Đã bán",
