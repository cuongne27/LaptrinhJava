  ...
  const mapDealerContractsToRows = useCallback(
    (contracts: DealerContract[]): InventoryTableRow[] => {
      return contracts.map((contract) => ({
        ...
        condition: contract.value ? `${contract.value.toLocaleString()} VNĐ` : "-",
        arrivalDate: new Date(contract.startDate).toLocaleDateString("vi-VN"),
        dealerName: contract.status,
        priceLabel: contract.value ? `${contract.value.toLocaleString()} VNĐ` : "-",
    ...
    async (page: number) => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error("Không thể tải dữ liệu hợp đồng. Vui lòng thử lại sau.");
        }
      ...
      <main className="flex-1 py-10">
        <Container className="flex flex-col gap-6">
          <div className="relative z-10">
            <FilterBar
              searchPlaceholder="Tìm kiếm theo số hợp đồng, tên đại lý..."
